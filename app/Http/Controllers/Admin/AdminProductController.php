...
class AdminProductController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($name = $request->name) $products->where('pro_name','pro_name_sale','pro_name_chay','like', '%'.$name.'%');
    ...
    {
        $data = $request->except('_token','pro_avatar','pro_avatar1','pro_avatar2','pro_avatar3','pro_avatar4','pro_avatar5','attribute','keywords','file');
        $data['pro_slug']     = Str::slug($request->pro_name);
        $data['created_at']   = Carbon::now();
        $path = $request->file('pro_name_chay')->store('images');
        ...
        // $avatarFields = ['pro_avatar', 'pro_avatar1', 'pro_avatar2', 'pro_avatar3', 'pro_avatar4', 'pro_avatar5'];

        //     foreach ($avatarFields as $field) {
        //         if ($request->hasFile($field)) {
        //         $image = upload_image($field);
        //         if ($image['code'] == 1) {
        //         $data[$field] = $image['name'];
        //     }
        // } }
    ...
    public function update(AdminRequestProduct $request, $id)
    {
        ...
        if ($request->pro_avatar) {
            $image = upload_image('pro_avatar','pro_avartar1');
            if ($image['code'] == null)
                $data['pro_avatar'] = $image['name'];
        }
